...
// Register a new handle. Must be called before calling rawhttps_server_listen.
// The pattern defines how rawhttps will treat the handle. If the pattern ends with a slash (/), the handle will be called
...
//
// server (input): Initialized rawhttps_server
// pattern (input): The pattern to be registered.
// pattern_size (input): The size, in bytes, of the pattern being registered
// handle (input): The callback function that rawhttps will call when this handle is triggered
...
// call rawhttps_server_destroy to shutdown the server when exiting
//
// server (input): Initialized rawhttps_server
