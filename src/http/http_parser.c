...
static long long rawhttps_parser_fetch_next_chunk(rawhttps_http_parser_buffer* hpb, int connected_socket, rawhttps_tls_state* ts)
{
...
// lets make rawhttps also have this function to make the parser a bit better
static int rawhttps_parser_fetch_next_byte(rawhttps_http_parser_buffer* hpb, int connected_socket, char* c, rawhttps_tls_state* ts)
{
	while (hpb->header_size + 1 > hpb->buffer_end)
		if (rawhttps_parser_fetch_next_chunk(hpb, connected_socket, ts) == -1)
			return -1;
...
static int rawhttps_parser_fetch_header(rawhttps_http_parser_buffer* hpb, int connected_socket, rawhttps_tls_state* ts)
{
	char c;

	for (;;)
	{
		if (rawhttps_parser_fetch_next_byte(hpb, connected_socket, &c, ts))
			return -1;
...
static int rawhttps_parser_get_next_bytes(rawhttps_http_parser_buffer* hpb, long long num, int connected_socket, char** ptr)
{
...
static int rawhttps_parser_get_next_string(rawhttps_http_parser_buffer* hpb, int connected_socket, char** string_ptr, long long* string_size)
{
	char* ptr;
	if (rawhttps_parser_get_next_bytes(hpb, 1, connected_socket, &ptr))
		return -1;
...
static int rawhttps_parser_get_request_header(rawhttps_http_parser_buffer* hpb, int connected_socket, char** request_header_ptr,
...
static int rawhttps_parser_end_of_header(rawhttps_http_parser_buffer* hpb)
{
...
{
	if (rawhttps_parser_fetch_header(&hps->hpb, connected_socket, hps->ts))
		return -1;
	...
	for (;;)
	{
		if (rawhttps_parser_end_of_header(&hps->hpb))
			break;
		...
		if (rawhttps_parser_get_request_header(&hps->hpb, connected_socket, &request_header, &request_header_size,
